  ...
  <div class="container-fluid">
	...
	<%
		...
		try {
			...
			String compare = request.getParameter("similar");
			String str = "SELECT s.UNITID, s.INSTNM, s.CITY, s.STABBR, s.INSTURL, s.ADM_RATE, s.SAT_AVG, s.UGDS, s.TUITIONFEE_IN, s.TUITIONFEE_OUT, s.UGDS_MEN, s.UGDS_WOMEN FROM schoolData s, schoolData s2 WHERE s.UNITID <> s2.UNITID AND s2.UNITID = ";

			str = str + item;
			
			if(compare.equals("sat")) {
				str = str + " AND ABS(s2.SAT_AVG - s.SAT_AVG) <= 50 ORDER BY ABS(s2.SAT_AVG - s.SAT_AVG) ASC";
			} else if(compare.equals("adm")) {
				str = str + " AND ABS(s2.ADM_RATE - s.ADM_RATE) <= 0.01 ORDER BY ABS(s2.ADM_RATE - s.ADM_RATE) ASC";
			} else if(compare.equals("tuit")) {
				str = str + " AND ABS(s2.TUITIONFEE_IN - s.TUITIONFEE_IN) <= 500 ORDER BY ABS(s2.TUITIONFEE_IN - s.TUITIONFEE_IN) ASC";
			} else if(compare.equals("num")) {
				str = str + " AND ABS(s2.UGDS - s.UGDS) <= 1000 ORDER BY ABS(s2.UGDS - s.UGDS) ASC";
			}
